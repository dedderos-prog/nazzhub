...
CLI_UC="$NAZZHUB_BIN"
UPDATER="$ROOT_DIR/nazzhub/files/usr/lib/components/updater.uc"
UPDATES_UC="$ROOT_DIR/nazzhub/files/usr/lib/components/updates.uc"
ACTION_UC="$ROOT_DIR/nazzhub/files/usr/lib/components/action.uc"
WORK_DIR="$(mktemp -d)"
...
[ ! -e "$ROOT_DIR/nazzhub/files/usr/lib/updater.sh" ] ||
...
NAZZHUB_LIB="$package_runtime_lib" \
FORKOP_RUNTIME_STATE_DIR="$WORK_DIR/package-runtime" \
NAZZHUB_BIN="$WORK_DIR/missing-forkop" \
...
release_json="$(cat <<'JSON'
{
  ...
  "assets": [
    ...
    {"name": "luci-app-nazzhub_1.2.3.ipk", "browser_download_url": "https://example.com/app.ipk"},
    {"name": "luci-i18n-nazzhub-ru_1.2.3.ipk", "browser_download_url": "https://example.com/i18n.ipk"}
  ]
}
JSON
)"

assert_eq "$(printf 'https://example.com/release\tforkop_1.2.3.ipk\thttps://example.com/backend.ipk\tluci-app-nazzhub_1.2.3.ipk\thttps://example.com/app.ipk\tluci-i18n-nazzhub-ru_1.2.3.ipk\thttps://example.com/i18n.ipk')" \
  "$(printf '%s' "$release_json" | ucode "$UPDATER" forkop-release-plan 1.2.3 ipk 1)" \
  "forkop release plan with i18n"
assert_eq "$(printf 'https://example.com/release\tforkop_1.2.3.ipk\thttps://example.com/backend.ipk\tluci-app-nazzhub_1.2.3.ipk\thttps://example.com/app.ipk\t\t')" \
...
cp "$NAZZHUB_LIB/core/uci.uc" "$fake_lib/core/uci.uc"
cp "$NAZZHUB_LIB/core/common.uc" "$fake_lib/core/common.uc"
...
start_json="$(NAZZHUB_LIB="$fake_lib" UPDATES_JOB_DIR="$component_actions_dir" \
  ucode -L "$NAZZHUB_LIB" "$UPDATES_UC" component-action-async sing_box check_update)"
...
for _ in 1 2 3 4 5; do
  status_json="$(NAZZHUB_LIB="$fake_lib" UPDATES_JOB_DIR="$component_actions_dir" \
    ucode -L "$NAZZHUB_LIB" "$UPDATES_UC" component-action-status "$job_id")"
...
start_json="$(NAZZHUB_LIB="$fake_lib" UPDATES_JOB_DIR="$component_actions_dir" \
  ucode -L "$NAZZHUB_LIB" "$UPDATES_UC" component-action-async sing-box check_update)"
job_id="$(JSON_VALUE="$start_json" node - <<'NODE'
const value = JSON.parse(process.env.JSON_VALUE);
if (!value.success || !value.job_id) {
  process.exit(1);
