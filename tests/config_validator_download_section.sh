...
VALIDATOR="$ROOT_DIR/nazzhub/files/usr/lib/config/validator.uc"
...
assert_accepts() {
  ...
  rows "$@" | ucode -L "$NAZZHUB_LIB" "$VALIDATOR" validate-download-section "$target" "$byedpi" "$zapret" "$zapret2"
}
...
assert_rejects() {
  ...
  if output="$(rows "$@" | ucode -L "$NAZZHUB_LIB" "$VALIDATOR" validate-download-section "$target" "$byedpi" "$zapret" "$zapret2" 2>/dev/null)"; then
