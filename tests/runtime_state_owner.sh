...
NAZZHUB_INIT="$ROOT_DIR/nazzhub/files/etc/init.d/nazzhub"
CLI_UC="$NAZZHUB_BIN"
RUNTIME_STATE="$ROOT_DIR/nazzhub/files/usr/lib/runtime_state.sh"
UPDATES_RUNTIME="$ROOT_DIR/nazzhub/files/usr/lib/updates_runtime.sh"
LIFECYCLE_UC="$ROOT_DIR/nazzhub/files/usr/lib/service/lifecycle.uc"
UPDATES_UC="$ROOT_DIR/nazzhub/files/usr/lib/components/updates.uc"
STATE_UC="$ROOT_DIR/nazzhub/files/usr/lib/service/state.uc"
NFT_UC="$ROOT_DIR/nazzhub/files/usr/lib/nft/apply.uc"
...
grep -Fq 'runtime_state.sh' "$NAZZHUB_BIN" &&
...
if grep -R -n -E "$service_predicates" "$NAZZHUB_BIN" "$ROOT_DIR/nazzhub/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
if grep -R -n -E "$pending_state" "$NAZZHUB_BIN" "$NAZZHUB_INIT" "$ROOT_DIR/nazzhub/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
if grep -R -n -E "$runtime_state_shell_symbols" "$NAZZHUB_BIN" "$NAZZHUB_INIT" "$ROOT_DIR/nazzhub/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
  fail "runtime_state.sh symbols must not remain in shell"
fi

if grep -Fq 'acquire_reload_lock' "$NAZZHUB_INIT" || grep -Fq 'release_reload_lock' "$NAZZHUB_INIT"; then
...
if grep -R -n -F 'updates_runtime.sh' "$NAZZHUB_BIN" "$ROOT_DIR/nazzhub/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
