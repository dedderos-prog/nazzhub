...
CLI_UC="$NAZZHUB_BIN"
SING_BOX_RUNTIME_SH="$NAZZHUB_LIB/sing_box_runtime.sh"
LIFECYCLE_UC="$NAZZHUB_LIB/service/lifecycle.uc"
SINGBOX_RUNTIME_UC="$NAZZHUB_LIB/singbox/runtime.uc"
SINGBOX_GENERATOR_UC="$NAZZHUB_LIB/singbox/generator.uc"
WORK_DIR="$(mktemp -d)"
...
if grep -R -n -E 'sing_box_runtime_ucode|rulesets_ucode|sing_box_configure_service|sing_box_init_config|get_service_listen_address|get_device_ipv4_address|get_download_detour_tag' "$NAZZHUB_BIN" "$NAZZHUB_LIB" --include='*.sh' >/dev/null 2>&1; then
...
stable_variant="$({
  PATH="$WORK_DIR/bin:$PATH" \
    ...
    ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" variant
...
if PATH="$WORK_DIR/check-bin:$PATH" \
  ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" check-config-fixture \
...
generator_reason="$(ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" generator-failure-reason-fixture \
...
ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" save-config-file-fixture \
...
if ucode -L "$NAZZHUB_LIB" "$SINGBOX_GENERATOR_UC" generate-config-fixture \
