...
NAZZHUB_CONFIG="$ROOT_DIR/nazzhub/files/etc/config/nazzhub"
...
require_make_dep() {
  ...
  grep -Eq "DEPENDS:=.*(^|[[:space:]])\\+$package([[:space:]]|$)" "$NAZZHUB_MAKEFILE" ||
    fail "nazzhub/Makefile DEPENDS is missing +$package"
}
...
require_file "$NAZZHUB_MAKEFILE"
require_file "$NAZZHUB_CONFIG"
require_file "$BUILD_SCRIPT"
require_file "$BUILD_WORKFLOW"
require_file "$NAZZHUB_LIB"
...
for conflict in https-dns-proxy nextdns luci-app-passwall luci-app-passwall2; do
  grep -E 'CONFLICTS:=' "$NAZZHUB_MAKEFILE" | grep -Fq "$conflict" ||
    fail "nazzhub/Makefile conflicts are missing $conflict"
...
if grep -Fq 'coreutils-sort' "$NAZZHUB_MAKEFILE" "$BUILD_SCRIPT"; then
  fail "unused coreutils-sort runtime dependency must not be packaged"
fi

grep -Fq "must use x.y.z format" "$NAZZHUB_MAKEFILE" ||
  fail "nazzhub/Makefile must enforce the three-part release version contract"
