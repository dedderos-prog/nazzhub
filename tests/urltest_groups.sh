...
PARSER_UC="$NAZZHUB_LIB/subscription/parser.uc"
GENERATOR_UC="$NAZZHUB_LIB/singbox/generator.uc"
CACHE_UC="$NAZZHUB_LIB/subscription/cache.uc"
WORK_DIR="$(mktemp -d)"
...
normalize_subscription() {
  local input="$1"
  local output="$2"
  ucode -L "$NAZZHUB_LIB" "$PARSER_UC" normalize-content "$input" "$output"
}
...
generate_config() {
  ...
  TMP_SUBSCRIPTION_FOLDER="$WORK_DIR/subscriptions" \
    ucode -L "$NAZZHUB_LIB" "$GENERATOR_UC" generate-config-fixture \
...
ucode -L "$NAZZHUB_LIB" "$CACHE_UC" get-outbound-metadata "$xray_config.section-cache" proxy "$WORK_DIR/missing-outbound-metadata.json" >"$xray_metadata"
...
ucode -L "$NAZZHUB_LIB" "$CACHE_UC" get-outbound-metadata "$singbox_config.section-cache" proxy "$WORK_DIR/missing-outbound-metadata.json" >"$singbox_metadata"
