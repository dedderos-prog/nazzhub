...
CLI_UC="$NAZZHUB_BIN"
HELPERS_SH="$NAZZHUB_LIB/helpers.sh"
LIFECYCLE_UC="$NAZZHUB_LIB/service/lifecycle.uc"
PACKAGES_UC="$NAZZHUB_LIB/core/packages.uc"
RULES_UC="$NAZZHUB_LIB/providers/rules.uc"
SINGBOX_RUNTIME_UC="$NAZZHUB_LIB/singbox/runtime.uc"
COMPONENT_ACTION_UC="$NAZZHUB_LIB/components/action.uc"
DIAGNOSTICS_RUNTIME_UC="$NAZZHUB_LIB/diagnostics/runtime.uc"
WORK_DIR="$(mktemp -d)"
...
if grep -R -n -F 'helpers.sh' "$NAZZHUB_BIN" "$NAZZHUB_LIB" --include='*.sh' >/dev/null 2>&1; then
...
if grep -R -n -E 'helpers_ucode\(|get_(inbound|server_inbound|tailscale_dns_server|outbound)_tag_by_section\(|get_domain_resolver_tag\(|provider_status_ucode\(|is_ipv4\(|is_min_package_version\(|url_get_(scheme|userinfo|host|port|path|query_param)\(' \
  "$NAZZHUB_BIN" "$NAZZHUB_LIB" --include='*.sh' >/dev/null 2>&1; then
...
[ ! -e "$NAZZHUB_LIB/status_diagnostics.sh" ] ||
...
mark_hex="$(ucode -L "$NAZZHUB_LIB" "$RULES_UC" mark-hex 0x01000000 2)"
[ "$mark_hex" = "0x01000002" ] ||
  fail "providers/rules.uc mark math changed"

version="$(printf 'sing-box version 1.12.4-extended\nEnvironment: test\n' |
  ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" version-from-output)"
[ "$version" = "1.12.4-extended" ] ||
  fail "singbox/runtime.uc version-from-output changed"

ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" is-extended "1.12.4-extended" >/dev/null ||
  fail "extended sing-box version should be detected"
if ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" is-extended "1.12.4" >/dev/null 2>&1; then
  fail "stable sing-box version must not be detected as extended"
fi
ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" supports-tailscale "" "$(printf 'Tags: with_quic,with_tailscale\n')" >/dev/null ||
  fail "with_tailscale build tag should be detected"

SB_VERSION_STATE_FILE="$WORK_DIR/version" \
  ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" write-version-state 1.2.3 >/dev/null ||
  fail "version state write failed"
[ "$(SB_VERSION_STATE_FILE="$WORK_DIR/version" ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" read-version-state)" = "1.2.3" ] ||
  fail "version state read failed"
SB_VERSION_STATE_FILE="$WORK_DIR/version" \
  ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" restore-version-state "" >/dev/null ||
...
SB_VARIANT_STATE_FILE="$WORK_DIR/variant" \
  ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" write-variant-marker extended-compressed >/dev/null ||
  fail "variant marker write failed"
SB_VARIANT_STATE_FILE="$WORK_DIR/variant" \
  ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" marker-is extended-compressed >/dev/null ||
  fail "variant marker check failed"
[ "$(SB_VARIANT_STATE_FILE="$WORK_DIR/variant" ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" read-variant-marker)" = "extended-compressed" ] ||
  fail "variant marker read failed"
SB_VARIANT_STATE_FILE="$WORK_DIR/variant" \
  ucode -L "$NAZZHUB_LIB" "$SINGBOX_RUNTIME_UC" restore-variant-marker "" >/dev/null ||
