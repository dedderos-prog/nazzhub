...
NAZZHUB_LIB="$NAZZHUB_LIB" ucode -L "$NAZZHUB_LIB" "$VALIDATOR" validate-runtime-fixture "$WORK_DIR/fixture.json" "$(context_json)"

if NAZZHUB_LIB="$NAZZHUB_LIB" ucode -L "$NAZZHUB_LIB" "$VALIDATOR" validate-runtime-fixture "$WORK_DIR/fixture.json" "$(context_json "0x01010000")" >/dev/null 2>&1; then
  fail "legacy Zapret2 route mark base should overlap Tailscale fwmark mask"
fi
if NAZZHUB_LIB="$NAZZHUB_LIB" ucode -L "$NAZZHUB_LIB" "$VALIDATOR" validate-runtime-fixture "$WORK_DIR/fixture.json" "$(context_json "" "0x00100000")" >/dev/null 2>&1; then
  fail "legacy FakeIP mark should overlap Tailscale fwmark mask"
fi
if NAZZHUB_LIB="$NAZZHUB_LIB" ucode -L "$NAZZHUB_LIB" "$VALIDATOR" validate-runtime-fixture "$WORK_DIR/fixture.json" "$(context_json "" "" "0x00200000")" >/dev/null 2>&1; then
