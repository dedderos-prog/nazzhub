...
if grep -R -n -E "$ui_runtime_shell_symbols" "$NAZZHUB_BIN" "$NAZZHUB_INIT" "$FORKOP_FILES/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
printf 'UI runtime job checks passed\n'
