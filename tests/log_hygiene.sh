...
reject_runtime_pattern() {
  ...
  if rg -n --fixed-strings "$pattern" "$ROOT_DIR/nazzhub/files" >/dev/null 2>&1; then
    rg -n --fixed-strings "$pattern" "$ROOT_DIR/nazzhub/files" >&2
    fail "$label"
  fi
}
...
reject_runtime_regex() {
  ...
  if rg -n "$pattern" "$ROOT_DIR/nazzhub/files" >/dev/null 2>&1; then
    rg -n "$pattern" "$ROOT_DIR/nazzhub/files" >&2
    fail "$label"
  fi
}
