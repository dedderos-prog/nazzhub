...
FAILOVER="$NAZZHUB_LIB/singbox/dns_failover.uc"
WORK_DIR="$(mktemp -d)"
...
selected="$(ucode -L "$NAZZHUB_LIB" "$FAILOVER" select-fixture "$WORK_DIR/select-state.json" "$WORK_DIR/alive-failover.json" main 0)"
...
selected="$(ucode -L "$NAZZHUB_LIB" "$FAILOVER" select-fixture "$WORK_DIR/select-state.json" "$WORK_DIR/alive-recovery.json" bootstrap 1)"
...
verification="$(ucode -L "$NAZZHUB_LIB" "$FAILOVER" verification-plan-fixture "$WORK_DIR/verify-previous.json" "$WORK_DIR/verify-bootstrap.json")"
