...
PACKAGE_UC="$NAZZHUB_LIB/service/package.uc"
NAZZHUB_MAKEFILE="$ROOT_DIR/nazzhub/Makefile"
LUCI_UCI_DEFAULTS="$ROOT_DIR/luci-app-nazzhub/root/etc/uci-defaults/50_luci-nazzhub"
...
grep -Fq 'package_prerm: [ "service/package.uc", "prerm", 1 ]' "$NAZZHUB_BIN" ||
  fail "forkop entrypoint must dispatch package prerm cleanup through service/package.uc"
grep -Fq 'package_postinst: [ "service/package.uc", "postinst", 0 ]' "$NAZZHUB_BIN" ||
  fail "forkop entrypoint must dispatch package postinst recovery through service/package.uc"
grep -Fq 'luci_postinst: [ "service/package.uc", "luci-postinst", 0 ]' "$NAZZHUB_BIN" ||
...
if grep -n -E 'grep -q "105 forkop"|sed -i "/105 forkop|forkop_dont_touch_dhcp=.*uci|cp /etc/config/nazzhub|rm -f /tmp/luci-indexcache|killall -HUP rpcd' "$NAZZHUB_MAKEFILE" "$BUILD_SCRIPT" >/dev/null; then
  fail "package scripts must not keep backend/LuCI lifecycle business logic in shell"
fi
grep -Fq '#!/usr/bin/ucode' "$NAZZHUB_MAKEFILE" ||
  fail "forkop Makefile package hooks must use ucode entrypoints"
grep -Fq '/usr/bin/nazzhub package_prerm' "$NAZZHUB_MAKEFILE" ||
  fail "forkop Makefile prerm must delegate cleanup to package_prerm"
grep -Fq '/usr/bin/nazzhub package_postinst' "$NAZZHUB_MAKEFILE" ||
  fail "forkop Makefile postinst must restore a service that was running before upgrade"
grep -Fq '/usr/bin/nazzhub package_prerm upgrade' "$BUILD_SCRIPT" ||
  fail "manual APK pre-upgrade must record and stop the running service"
grep -Fq '/usr/bin/nazzhub package_postinst' "$BUILD_SCRIPT" ||
  fail "manual packages must restore a service that was running before upgrade"
if grep -Fq '/usr/bin/nazzhub luci_postinst' "$BUILD_SCRIPT"; then
...
if grep -n -E 'Package/forkop/preinst|copy_legacy_config|FORKOP_LEGACY_CONFIG|mode == "preinst"' \
  "$NAZZHUB_MAKEFILE" "$BUILD_SCRIPT" "$PACKAGE_UC" >/dev/null 2>&1; then
...
FORKOP_PACKAGE_TEST_MODE=1 FORKOP_RT_TABLES="$rt_tables" \
  ucode -L "$NAZZHUB_LIB" "$PACKAGE_UC" prerm
...
NAZZHUB_INIT="$WORK_DIR/forkop-init" \
FORKOP_STOP_LOG="$WORK_DIR/stop-order.log" \
NAZZHUB_BIN="$WORK_DIR/missing-forkop-bin" \
...
FORKOP_PACKAGE_TEST_MODE=1 FORKOP_LUCI_CACHE_GLOBS="$WORK_DIR/luci-indexcache*" \
  ucode -L "$NAZZHUB_LIB" "$PACKAGE_UC" luci-postinst
...
NAZZHUB_BIN="$WORK_DIR/forkop-bin" \
FORKOP_DNS_APPLY_UC="$WORK_DIR/missing-dns-apply.uc" \
FORKOP_SING_BOX_INIT="$WORK_DIR/missing-sing-box-init" \
FORKOP_SING_BOX_BIN="$WORK_DIR/missing-sing-box-bin" \
FORKOP_SING_BOX_CRONET="$WORK_DIR/missing-cronet" \
