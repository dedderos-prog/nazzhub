...
VALIDATOR_UC="$NAZZHUB_LIB/config/validator.uc"
PRIORITY_UC="$NAZZHUB_LIB/singbox/priority.uc"
WORK_DIR="$(mktemp -d)"
...
JS
  NAZZHUB_LIB="$NAZZHUB_LIB" ucode -L "$NAZZHUB_LIB" "$VALIDATOR_UC" \
...
{ "a": -1, "b": 200, "c": 50 }
JSON
first_live="$(ucode -L "$NAZZHUB_LIB" "$PRIORITY_UC" select-fixture \
...
{ "a": 80, "b": 20, "c": 50 }
JSON
fastest="$(ucode -L "$NAZZHUB_LIB" "$PRIORITY_UC" select-fixture \
...
skip_dead="$(ucode -L "$NAZZHUB_LIB" "$PRIORITY_UC" select-fixture \
...
{ "a": 100, "b": 50, "c": 80 }
JSON
same_level="$(ucode -L "$NAZZHUB_LIB" "$PRIORITY_UC" select-faster-fixture \
