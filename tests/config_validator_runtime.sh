...
JS
  NAZZHUB_LIB="$NAZZHUB_LIB" ucode -L "$NAZZHUB_LIB" "$VALIDATOR" validate-runtime-fixture "$normalized" "$context"
}
...
JSON
if output="$(NAZZHUB_LIB="$NAZZHUB_LIB" ucode -L "$NAZZHUB_LIB" "$VALIDATOR" validate-runtime-fixture "$WORK_DIR/bad-byedpi.json" "$provider_context" 2>/dev/null)"; then
...
mkdir -p "$runtime_lib"
ln -s "$NAZZHUB_LIB/core" "$runtime_lib/core"
ln -s "$NAZZHUB_LIB/config" "$runtime_lib/config"
ln -s "$NAZZHUB_LIB/subscription" "$runtime_lib/subscription"
ln -s "$NAZZHUB_LIB/providers" "$runtime_lib/providers"
