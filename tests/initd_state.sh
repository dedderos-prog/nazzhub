...
INITD_UC="$NAZZHUB_LIB/service/initd.uc"
STATE_UC="$NAZZHUB_LIB/service/state.uc"
INITD="$ROOT_DIR/nazzhub/files/etc/init.d/nazzhub"
WORK_DIR="$(mktemp -d)"
...
if grep -Fq 'procd_set_param command "$NAZZHUB_BIN" start' "$INITD"; then
