...
CACHE_UC="$ROOT_DIR/nazzhub/files/usr/lib/subscription/cache.uc"
...
SUBSCRIPTION_RUNTIME_SH="$ROOT_DIR/nazzhub/files/usr/lib/subscription_runtime.sh"
UPDATES_RUNTIME_SH="$ROOT_DIR/nazzhub/files/usr/lib/updates_runtime.sh"
...
if grep -R -n "subscription_runtime.sh" "$ROOT_DIR/nazzhub/files" >/dev/null 2>&1; then
...
if grep -R -n -E "$subscription_runtime_shell_symbols" "$ROOT_DIR/nazzhub/files/usr/bin/nazzhub" "$ROOT_DIR/nazzhub/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
  fail "subscription_runtime shell symbols must not remain in runtime shell"
fi

if grep -R -n -E 'get_subscription_metadata_path|get_outbound_metadata_path' "$ROOT_DIR/nazzhub/files/usr/bin/nazzhub" "$ROOT_DIR/nazzhub/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
assert_eq "proxy-subscription-2" \
  ...
  "source id mode"
assert_eq "/var/run/nazzhub/subscription-metadata/proxy.json" \
  "$(cache_ucode subscription-metadata-path proxy)" \
  "subscription metadata path mode"
assert_eq "/var/run/nazzhub/outbound-metadata/proxy.json" \
