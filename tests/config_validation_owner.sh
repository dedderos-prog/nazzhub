...
legacy_symbols='(^|[^A-Za-z0-9_])(config_validate_runtime|check_requirements|commit_NAZZHUB_CONFIG|mwan3_is_active|get_inline_remote_ruleset_format|detect_inline_ruleset_reference_kind)([^A-Za-z0-9_]|$)'
if grep -R -n -E "$legacy_symbols" "$NAZZHUB_BIN" "$NAZZHUB_LIB" --include='*.sh' >/dev/null 2>&1; then
