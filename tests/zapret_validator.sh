...
VALIDATOR="$ROOT_DIR/nazzhub/files/usr/lib/providers/zapret/validator.uc"
ZAPRET2_VALIDATOR="$ROOT_DIR/nazzhub/files/usr/lib/providers/zapret2/validator.uc"
WORK_DIR="$(mktemp -d)"
...
valid_nfqws="$(ucode -L "$NAZZHUB_LIB" -- "$VALIDATOR" validate-json nfqws '--dpi-desync=fake --dpi-desync-repeats 2')"
assert_json_field "$valid_nfqws" valid true

configured="$(ucode -L "$NAZZHUB_LIB" -- "$VALIDATOR" strategy-or-default "$(printf -- '--dpi-desync=fake\t--dpi-desync-repeats 2')" '--default')"
[ "$configured" = "--dpi-desync=fake --dpi-desync-repeats 2" ] ||
  fail "configured zapret strategy should be normalized, got '$configured'"

defaulted="$(ucode -L "$NAZZHUB_LIB" -- "$VALIDATOR" strategy-or-default "" "$(printf -- '--default\t1')")"
[ "$defaulted" = "--default 1" ] ||
  fail "empty zapret strategy should use normalized default, got '$defaulted'"

if invalid_nfqws="$(ucode -L "$NAZZHUB_LIB" -- "$VALIDATOR" validate-json nfqws '--hostlist domains.txt' 2>/dev/null)"; then
...
ucode -L "$NAZZHUB_LIB" "$WORK_DIR/require-zapret-validator.uc"
