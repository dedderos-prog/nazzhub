...
CLI_UC="$NAZZHUB_BIN"
BYEDPI_RUNTIME_SH="$NAZZHUB_LIB/byedpi.sh"
LIFECYCLE_UC="$NAZZHUB_LIB/service/lifecycle.uc"
BYEDPI_RUNTIME_UC="$NAZZHUB_LIB/providers/byedpi/runtime.uc"
WORK_DIR="$(mktemp -d)"
...
version="$(NAZZHUB_LIB="$NAZZHUB_LIB" PATH="$WORK_DIR:$PATH" ucode -L "$NAZZHUB_LIB" "$BYEDPI_RUNTIME_UC" package-version)"
[ "$version" = "0.17.3-r1" ] ||
  fail "ByeDPI APK version was parsed as '$version'"

NAZZHUB_LIB="$NAZZHUB_LIB" BYEDPI_BIN="$ROOT_DIR/tests/missing-ciadpi" \
  ucode -L "$NAZZHUB_LIB" "$BYEDPI_RUNTIME_UC" check |
  node -e '
const fs = require("fs");
