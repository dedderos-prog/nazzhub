...
DIAGNOSTICS="$ROOT_DIR/nazzhub/files/usr/lib/diagnostics/status.uc"
DIAGNOSTICS_RUNTIME="$ROOT_DIR/nazzhub/files/usr/lib/diagnostics/runtime.uc"
...
status_ucode() {
  ucode -L "$NAZZHUB_LIB" "$DIAGNOSTICS" "$@"
}
...
printf '%s\n' "$sing_box_netstat" |
  NAZZHUB_LIB="$NAZZHUB_LIB" ucode -L "$NAZZHUB_LIB" "$DIAGNOSTICS_RUNTIME" sing-box-standard-ports-listening-fixture >/dev/null ||
  fail "sing-box standard listeners should satisfy diagnostics"
if printf '%s\n' "$sing_box_netstat" | sed '/0.0.0.0:1602/d' |
  NAZZHUB_LIB="$NAZZHUB_LIB" ucode -L "$NAZZHUB_LIB" "$DIAGNOSTICS_RUNTIME" sing-box-standard-ports-listening-fixture >/dev/null 2>&1; then
