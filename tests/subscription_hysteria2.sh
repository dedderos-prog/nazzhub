...
PARSER="$ROOT_DIR/nazzhub/files/usr/lib/subscription/parser.uc"
GENERATOR="$ROOT_DIR/nazzhub/files/usr/lib/singbox/generator.uc"
WORK_DIR="$(mktemp -d)"
...
TMP_SUBSCRIPTION_FOLDER="$WORK_DIR/subscriptions" \
  ucode -L "$NAZZHUB_LIB" "$GENERATOR" generate-config-fixture \
...
printf 'subscription Hysteria2 checks passed\n'
