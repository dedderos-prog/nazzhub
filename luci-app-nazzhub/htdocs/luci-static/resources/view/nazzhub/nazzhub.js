...
// Quick Setup / Subscription (First Page)
"require view.nazzhub.subscription as subscription";

// Global settings
"require view.nazzhub.settings as settings";

// Sections
"require view.nazzhub.section as section";

// Server
"require view.nazzhub.server as server";

// Dashboard
"require view.nazzhub.dashboard as dashboard";

// Monitoring
"require view.nazzhub.monitoring as monitoring";

// Diagnostic
"require view.nazzhub.diagnostic as diagnostic";
...
const EntryPoint = {
  async render() {
      ...
      uiCapabilitiesPromise = main.ForkopShellMethods.getUiCapabilities()
        ...
        .catch((error) => {
          ...
          return main.ForkopShellMethods.getUiState()
            ...
            .catch((fallbackError) => {
              console.warn("Failed to load NAZZHUB UI state", fallbackError);
    ...
    subscriptionSection.cfgsections = function () {
      const allSubSections = uci.sections(UCI_PACKAGE, "subscription_url") || [];
      const mainSub = allSubSections.find(
        (s) => s[".name"] === "main_sub" || s.section === "Main",
      );
      return mainSub
        ? [mainSub[".name"]]
