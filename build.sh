...
Build NAZZHUB IPK and APK packages. The version must use x.y.z format.
EOF
}
...
SDK_CACHE_DIR="${SDK_CACHE_DIR:-${XDG_CACHE_HOME:-$HOME/.cache}/nazzhub/openwrt-sdk}"
...
BACKEND_DESCRIPTION="Rule-based NAZZHUB backend with hybrid sing-box + zapret orchestration"
APP_DESCRIPTION="Rule-based NAZZHUB LuCI app with hybrid sing-box + zapret orchestration"
I18N_DESCRIPTION="Translation for luci-app-nazzhub - Русский (Russian)"
MAINTAINER="dedderos-prog <dedderos-prog@users.noreply.github.com>"
PROJECT_URL="https://github.com/dedderos-prog/nazzhub"
...
APP_DEPENDS_IPK="libc, luci-base, nazzhub"
APP_DEPENDS_APK="libc luci-base nazzhub"
...
extract_sdk() {
  ...
  local marker_file="$destination/.nazzhub-sdk-url"
  local temp_dir
...
build_backend_root() {
  ...
  make_dir "$output_root/usr/lib/nazzhub"

  install -m 0755 "$ROOT_DIR/nazzhub/files/etc/init.d/nazzhub" "$output_root/etc/init.d/nazzhub"
  install -m 0644 "$ROOT_DIR/nazzhub/files/etc/config/nazzhub" "$output_root/etc/config/nazzhub"
  install -m 0755 "$ROOT_DIR/nazzhub/files/usr/bin/nazzhub" "$output_root/usr/bin/nazzhub"
  cp -a "$ROOT_DIR/nazzhub/files/usr/lib/." "$output_root/usr/lib/nazzhub/"

  sed -i -e "s/__COMPILED_VERSION_VARIABLE__/${RELEASE_VERSION}/g" \
    "$output_root/usr/lib/nazzhub/core/constants.uc"

  normalize_package_root_modes "$output_root"
  chmod 0755 "$output_root/etc/init.d/nazzhub" "$output_root/usr/bin/nazzhub"
}
...
build_app_root() {
  ...
  cp -a "$ROOT_DIR/luci-app-nazzhub/htdocs/." "$output_root/www/"
  cp -a "$ROOT_DIR/luci-app-nazzhub/root/." "$output_root/"
  sed -i -e "s/__COMPILED_VERSION_VARIABLE__/${RELEASE_VERSION}/g" \
    "$output_root/www/luci-static/resources/view/nazzhub/main.js"
...
build_i18n_root() {
  local output_root="$1"
  local po2lmo_bin="$2"
  local lmo_path="$output_root/usr/lib/lua/luci/i18n/nazzhub.ru.lmo"

  rm -rf "$output_root"
  make_dir "$output_root/etc/uci-defaults"
  make_dir "$(dirname "$lmo_path")"

  cat > "$output_root/etc/uci-defaults/luci-i18n-nazzhub-ru" <<'EOF'
uci set luci.languages.ru='Русский (Russian)'; uci commit luci
EOF

  "$po2lmo_bin" "$ROOT_DIR/luci-app-nazzhub/po/ru/nazzhub.po" "$lmo_path"
...
write_i18n_ipk_control() {
  ...
  cat > "$control_dir/control" <<EOF
Package: luci-i18n-nazzhub-ru
Version: ${RELEASE_VERSION}
Depends: libc, luci-app-nazzhub
License: GPL-2.0-or-later
Section: luci
URL: ${PROJECT_URL}
Maintainer: ${MAINTAINER}
...
#!/usr/bin/ucode
if (getenv("IPKG_INSTROOT") == null || getenv("IPKG_INSTROOT") == "")
    exit(system("NAZZHUB_LIB=/usr/lib/nazzhub ucode -L /usr/lib/nazzhub /usr/lib/nazzhub/config/migration.uc migrate && /usr/bin/nazzhub package_postinst"));
exit(0);
EOF
...
if (getenv("IPKG_INSTROOT") == null || getenv("IPKG_INSTROOT") == "")
	system("/usr/bin/nazzhub package_prerm remove >/dev/null 2>&1");
...
#!/usr/bin/ucode
if (getenv("IPKG_INSTROOT") == null || getenv("IPKG_INSTROOT") == "")
    exit(system("/usr/bin/nazzhub package_prerm upgrade >/dev/null 2>&1"));
exit(0);
EOF
...
export pkgname="luci-app-nazzhub"
add_group_and_user
default_postinst
EOF
...
. ${IPKG_INSTROOT}/lib/functions.sh
export root="${IPKG_INSTROOT}"
export pkgname="luci-i18n-nazzhub-ru"
add_group_and_user
default_postinst
EOF
...
verify_ipk_metadata() {
  ...
  if [[ "$expected_package" == "nazzhub" ]]; then
...
main() {
  ...
  if ! flock -n 9; then
    echo "Another NAZZHUB package build is already running" >&2
    exit 1
  fi
  output_dir="$OUTPUT_DIR"
  mkdir -p "$output_dir"
  rm -f "$output_dir"/nazzhub_* "$output_dir"/luci-app-nazzhub_* "$output_dir"/luci-i18n-nazzhub-ru_*
  ...
  build_ipk_package \
    ...
    "$backend_root" \
    "$backend_control" \
    "$output_dir/nazzhub_${RELEASE_VERSION}.ipk"
  ...
  build_ipk_package \
    ...
    "$app_root" \
    "$app_control" \
    "$output_dir/luci-app-nazzhub_${RELEASE_VERSION}.ipk"

  build_ipk_package \
    "$ipkg_build_bin" \
    "luci-i18n-nazzhub-ru" \
    "$i18n_root" \
    "$i18n_control" \
    "$output_dir/luci-i18n-nazzhub-ru_${RELEASE_VERSION}.ipk"

  generate_apk_metadata_files "nazzhub" "$backend_root" "/etc/config/nazzhub"
  generate_apk_metadata_files "luci-app-nazzhub" "$app_root"
  generate_apk_metadata_files "luci-i18n-nazzhub-ru" "$i18n_root"
  ...
  build_apk_package \
    ...
    "$apk_scripts" \
    "backend" \
    "$output_dir/nazzhub_${RELEASE_VERSION}.apk" \
    "$MAINTAINER"
  ...
  build_apk_package \
    ...
    "$apk_scripts" \
    "app" \
    "$output_dir/luci-app-nazzhub_${RELEASE_VERSION}.apk" \
    "$MAINTAINER"
  ...
  build_apk_package \
    ...
    "$APK_INTERNAL_VERSION" \
    "$I18N_DESCRIPTION" \
    "libc luci-app-nazzhub" \
    "$i18n_root" \
    "$apk_scripts" \
    "i18n" \
    "$output_dir/luci-i18n-nazzhub-ru_${RELEASE_VERSION}.apk" \
    "$MAINTAINER"

  verify_ipk_metadata "$output_dir/nazzhub_${RELEASE_VERSION}.ipk" "nazzhub" "$RELEASE_VERSION"
  verify_ipk_metadata "$output_dir/luci-app-nazzhub_${RELEASE_VERSION}.ipk" "luci-app-nazzhub" "$RELEASE_VERSION"
  verify_ipk_metadata "$output_dir/luci-i18n-nazzhub-ru_${RELEASE_VERSION}.ipk" "luci-i18n-nazzhub-ru" "$RELEASE_VERSION"
  verify_apk_metadata "$apk_bin" "$output_dir/nazzhub_${RELEASE_VERSION}.apk" "nazzhub" "$APK_INTERNAL_VERSION"
  verify_apk_metadata "$apk_bin" "$output_dir/luci-app-nazzhub_${RELEASE_VERSION}.apk" "luci-app-nazzhub" "$APK_INTERNAL_VERSION"
  verify_apk_metadata "$apk_bin" "$output_dir/luci-i18n-nazzhub-ru_${RELEASE_VERSION}.apk" "luci-i18n-nazzhub-ru" "$APK_INTERNAL_VERSION"
