...
FETCHER=""
TMP_DIR=""
NAZZHUB_WAS_ENABLED=0
NAZZHUB_WAS_RUNNING=0
NAZZHUB_LEGACY_DETECTED=0
NAZZHUB_I18N_REQUESTED=0
INSTALLER_LANG="en"
SING_BOX_INSTALL_VARIANT=""

NAZZHUB_RELEASE_JSON=""
NAZZHUB_RELEASE_TAG=""
NAZZHUB_BACKEND_URL=""
NAZZHUB_BACKEND_NAME=""
NAZZHUB_BACKEND_FILE=""
NAZZHUB_APP_URL=""
NAZZHUB_APP_NAME=""
NAZZHUB_APP_FILE=""
NAZZHUB_I18N_URL=""
...
Installs or updates NAZZHUB packages:
  - NAZZHUB
  - luci-app-nazzhub
  - luci-i18n-nazzhub-ru when requested or when LuCI language is Russian
...
init_tmp_dir() {
    TMP_DIR="$(mktemp -d /tmp/nazzhub.XXXXXX 2>/dev/null || true)"
...
detect_fetcher() {
    ...
    fail "wget or curl is required to download NAZZHUB"
}

run_with_deadline() {
    NAZZHUB_deadline_seconds="$1"
    shift

    NAZZHUB_deadline_helper="${NAZZHUB_DEADLINE_HELPER_PATH:-}"
    if [ -z "$NAZZHUB_deadline_helper" ]; then
        NAZZHUB_deadline_helper="$(install_deadline_helper_path)" || return 1
    fi

    NAZZHUB_deadline_result="$TMP_DIR/deadline-result.$$"
    "$NAZZHUB_deadline_helper" run "$NAZZHUB_deadline_seconds" "$NAZZHUB_deadline_result" "$@"
    NAZZHUB_deadline_status=$?
    rm -f "$NAZZHUB_deadline_result.output" "$NAZZHUB_deadline_result.error" \
        "$NAZZHUB_deadline_result.status" "$NAZZHUB_deadline_result.timeout"
    return "$NAZZHUB_deadline_status"
}
...
const INSTALLER_NAZZHUB_INIT = env("NAZZHUB_INSTALLER_INIT", "/etc/init.d/nazzhub");
const INSTALLER_NAZZHUB_BIN = env("NAZZHUB_INSTALLER_BIN", "/usr/bin/nazzhub");
const INSTALLER_NAZZHUB_LIB = env("NAZZHUB_INSTALLER_LIB", "/usr/lib/nazzhub");
const INSTALLER_NAZZHUB_PERSISTENT_DIR = env("NAZZHUB_INSTALLER_PERSISTENT_DIR", "/etc/nazzhub");
const INSTALLER_NAZZHUB_UCI_DEFAULTS = env("NAZZHUB_INSTALLER_UCI_DEFAULTS", "/etc/uci-defaults/50_luci-nazzhub");
const INSTALLER_NAZZHUB_LUCI_VIEW = env("NAZZHUB_INSTALLER_LUCI_VIEW", "/www/luci-static/resources/view/nazzhub");
const INSTALLER_MENU_JSON = env("NAZZHUB_INSTALLER_MENU_JSON", "/usr/share/luci/menu.d/luci-app-nazzhub.json");
const INSTALLER_ACL_JSON = env("NAZZHUB_INSTALLER_ACL_JSON", "/usr/share/rpcd/acl.d/luci-app-nazzhub.json");
const INSTALLER_RU_LMO = env("NAZZHUB_INSTALLER_RU_LMO", "/usr/lib/lua/luci/i18n/nazzhub.ru.lmo");
const INSTALLER_EN_LMO = env("NAZZHUB_INSTALLER_EN_LMO", "/usr/lib/lua/luci/i18n/nazzhub.en.lmo");
const INSTALLER_RU_LUA = env("NAZZHUB_INSTALLER_RU_LUA", "/usr/lib/lua/luci/i18n/nazzhub.ru.lua");
const INSTALLER_EN_LUA = env("NAZZHUB_INSTALLER_EN_LUA", "/usr/lib/lua/luci/i18n/nazzhub.en.lua");
const INSTALLER_RPCD_INIT = env("NAZZHUB_INSTALLER_RPCD_INIT", "/etc/init.d/rpcd");
const LEGACY_BRAND = env("NAZZHUB_INSTALLER_LEGACY_BRAND", "");
const LEGACY_BACKEND_PACKAGE = env("NAZZHUB_INSTALLER_LEGACY_BACKEND", LEGACY_BRAND + "-plus");
const LEGACY_CONFIG_PACKAGE_ALT = env("NAZZHUB_INSTALLER_LEGACY_CONFIG_ALT", LEGACY_BRAND + "_plus");
const INSTALLER_LEGACY_INIT = env("NAZZHUB_INSTALLER_LEGACY_INIT", "/etc/init.d/" + LEGACY_BACKEND_PACKAGE);
const INSTALLER_LEGACY_BASE_INIT = env("NAZZHUB_INSTALLER_LEGACY_BASE_INIT", "/etc/init.d/" + LEGACY_BRAND);
const INSTALLER_LEGACY_BASE_BIN = env("NAZZHUB_INSTALLER_LEGACY_BASE_BIN", "/usr/bin/" + LEGACY_BRAND);
const INSTALLER_LEGACY_BASE_LIB = env("NAZZHUB_INSTALLER_LEGACY_BASE_LIB", "/usr/lib/" + LEGACY_BRAND);
const INSTALLER_LEGACY_BASE_UCI_DEFAULTS = env("NAZZHUB_INSTALLER_LEGACY_BASE_UCI_DEFAULTS", "/etc/uci-defaults/50_luci-" + LEGACY_BRAND);
const INSTALLER_LEGACY_BASE_LUCI_VIEW = env("NAZZHUB_INSTALLER_LEGACY_BASE_LUCI_VIEW", "/www/luci-static/resources/view/" + LEGACY_BRAND);
const INSTALLER_LEGACY_BASE_MENU_JSON = env("NAZZHUB_INSTALLER_LEGACY_BASE_MENU_JSON", "/usr/share/luci/menu.d/luci-app-" + LEGACY_BRAND);
const INSTALLER_LEGACY_BASE_ACL_JSON = env("NAZZHUB_INSTALLER_LEGACY_BASE_ACL_JSON", "/usr/share/rpcd/acl.d/luci-app-" + LEGACY_BRAND);
const INSTALLER_LEGACY_BASE_I18N = env("NAZZHUB_INSTALLER_LEGACY_BASE_I18N", "/usr/lib/lua/luci/i18n/" + LEGACY_BRAND);
const INSTALLER_LEGACY_BASE_CONFIG = env("NAZZHUB_INSTALLER_LEGACY_BASE_CONFIG", "/etc/config/" + LEGACY_BRAND);
const INSTALLER_LEGACY_BASE_PERSISTENT_DIR = env("NAZZHUB_INSTALLER_LEGACY_BASE_PERSISTENT_DIR", "/etc/" + LEGACY_BRAND);
const INSTALLER_LEGACY_BASE_RUNTIME_DIR = env("NAZZHUB_INSTALLER_LEGACY_BASE_RUNTIME_DIR", "/var/run/" + LEGACY_BRAND);
const INSTALLER_LEGACY_BASE_TMP_DIR = env("NAZZHUB_INSTALLER_LEGACY_BASE_TMP_DIR", "/tmp/" + LEGACY_BRAND);
const INSTALLER_LEGACY_TMP_PACKAGE_GLOB = env("NAZZHUB_INSTALLER_LEGACY_TMP_PACKAGE_GLOB", "/tmp/*" + LEGACY_BRAND + "*");
const INSTALLER_LEGACY_SCAN_ROOTS = env("NAZZHUB_INSTALLER_LEGACY_SCAN_ROOTS", "/tmp /var/run /etc /usr/lib /usr/share/luci /usr/share/rpcd /www/luci-static/resources/view");
const INSTALLER_LEGACY_BIN = env("NAZZHUB_INSTALLER_LEGACY_BIN", "/usr/bin/" + LEGACY_BACKEND_PACKAGE);
const INSTALLER_LEGACY_LIB = env("NAZZHUB_INSTALLER_LEGACY_LIB", "/usr/lib/" + LEGACY_BACKEND_PACKAGE);
const INSTALLER_LEGACY_UCI_DEFAULTS = env("NAZZHUB_INSTALLER_LEGACY_UCI_DEFAULTS", "/etc/uci-defaults/50_luci-" + LEGACY_BACKEND_PACKAGE);
const INSTALLER_LEGACY_LUCI_VIEW = env("NAZZHUB_INSTALLER_LEGACY_LUCI_VIEW", "/www/luci-static/resources/view/" + LEGACY_CONFIG_PACKAGE_ALT);
const INSTALLER_LEGACY_MENU_JSON = env("NAZZHUB_INSTALLER_LEGACY_MENU_JSON", "/usr/share/luci/menu.d/luci-app-" + LEGACY_BACKEND_PACKAGE + ".json");
const INSTALLER_LEGACY_ACL_JSON = env("NAZZHUB_INSTALLER_LEGACY_ACL_JSON", "/usr/share/rpcd/acl.d/luci-app-" + LEGACY_BACKEND_PACKAGE + ".json");
const INSTALLER_LEGACY_CONFIG = env("NAZZHUB_INSTALLER_LEGACY_CONFIG", "/etc/config/" + LEGACY_BACKEND_PACKAGE);
const INSTALLER_LEGACY_CONFIG_ALT = env("NAZZHUB_INSTALLER_LEGACY_CONFIG_FILE_ALT", "/etc/config/" + LEGACY_CONFIG_PACKAGE_ALT);
const INSTALLER_LEGACY_PERSISTENT_DIR = env("NAZZHUB_INSTALLER_LEGACY_PERSISTENT_DIR", "/etc/" + LEGACY_BACKEND_PACKAGE);
const INSTALLER_LEGACY_RUNTIME_DIR = env("NAZZHUB_INSTALLER_LEGACY_RUNTIME_DIR", "/var/run/" + LEGACY_BACKEND_PACKAGE);
const INSTALLER_LEGACY_TMP_DIR = env("NAZZHUB_INSTALLER_LEGACY_TMP_DIR", "/tmp/" + LEGACY_BACKEND_PACKAGE);
const INSTALLER_LEGACY_TMP_ALT_DIR = env("NAZZHUB_INSTALLER_LEGACY_TMP_ALT_DIR", "/tmp/" + LEGACY_CONFIG_PACKAGE_ALT);
const INSTALLER_DEADLINE_HELPER = env("NAZZHUB_INSTALLER_DEADLINE_HELPER", "");
const INSTALLER_COMMAND_RESULT = env("NAZZHUB_INSTALLER_COMMAND_RESULT", "/tmp/nazzhub-installer-command");
const INSTALLER_RC_DIR = env("NAZZHUB_INSTALLER_RC_DIR", "/etc/rc.d");
const INSTALLER_START_RETRY_FILE = env("NAZZHUB_INSTALLER_START_RETRY_FILE", "/var/run/nazzhub/start.retry");
const INSTALLER_START_RETRY_PID_FILE = env("NAZZHUB_INSTALLER_START_RETRY_PID_FILE", "/var/run/nazzhub/start-retry.pid");
const INSTALLER_ORPHAN_PPID = env("NAZZHUB_INSTALLER_ORPHAN_PPID", "1");
const INSTALLER_SERVICE_PROBE_TIMEOUT = int(env("NAZZHUB_INSTALLER_SERVICE_PROBE_TIMEOUT", "6")) || 6;
const INSTALLER_SERVICE_ACTION_TIMEOUT = int(env("NAZZHUB_INSTALLER_SERVICE_ACTION_TIMEOUT", "60")) || 60;
...
let dns_owner_config = "nazzhub";
let dns_owner_section = "settings";
let dns_owner_option_prefix = "nazzhub_";
...
function installer_cancel_stale_start_retry() {
    ...
    if (match(pid, /^[0-9]+$/)) {
        ...
        if (installer_args_contain(args, INSTALLER_NAZZHUB_INIT) &&
...
function select_dns_owner(legacy) {
    if (legacy) {
        ...
        dns_owner_config = "nazzhub";
        dns_owner_section = "settings";
        dns_owner_option_prefix = "nazzhub_";
    }
}
...
function installer_deactivate_legacy_base() {
    ...
    if (running.value) {
        warn("Detected a running legacy service. Stopping it before installing NAZZHUB.\n");
        if (!installer_service_action(INSTALLER_LEGACY_BASE_INIT, "stop"))
            return false;
    }

    if (enabled.value) {
        warn("Detected an enabled legacy autostart. Disabling it before installing NAZZHUB.\n");
...
function installer_cleanup_legacy() {
    let NAZZHUB_installed = installer_package_installed("nazzhub");
    let legacy_installed = LEGACY_BRAND != "" && installer_package_installed(LEGACY_BACKEND_PACKAGE);
    let active_init = legacy_installed ? INSTALLER_LEGACY_INIT : INSTALLER_NAZZHUB_INIT;
    let active_bin = legacy_installed ? INSTALLER_LEGACY_BIN : INSTALLER_NAZZHUB_BIN;

    installer_recover_interrupted_cleanup([
        active_init,
        INSTALLER_NAZZHUB_INIT,
    ...
    if (!enabled.known || (!running.known && !backend_running.known)) {
        warn("Unable to determine the NAZZHUB service state before installation.\n");
        return false;
    }
    ...
    if (!installer_remove_package_prefix("luci-i18n-nazzhub"))
        packages_removed = false;
    if (!installer_remove_package("luci-app-nazzhub"))
        packages_removed = false;
    ...
    if (!NAZZHUB_installed) {
        remove_path(INSTALLER_NAZZHUB_LIB);
        remove_path(INSTALLER_NAZZHUB_INIT);
        remove_path(INSTALLER_NAZZHUB_BIN);
    }

    for (let path in [
        INSTALLER_NAZZHUB_LUCI_VIEW,
        INSTALLER_MENU_JSON,
        INSTALLER_ACL_JSON,
        INSTALLER_NAZZHUB_UCI_DEFAULTS,
        INSTALLER_RU_LMO,
        INSTALLER_EN_LMO,
        INSTALLER_RU_LUA,
        INSTALLER_EN_LUA
    ...
    print("NAZZHUB_WAS_ENABLED=", was_enabled ? "1" : "0", "\n");
    print("NAZZHUB_WAS_RUNNING=", was_running ? "1" : "0", "\n");
    print("NAZZHUB_LEGACY_DETECTED=", legacy_installed ? "1" : "0", "\n");
    return true;
}
...
function installer_finalize_legacy() {
    ...
    if (path_exists(legacy_tailscale_dir)) {
        ...
        let NAZZHUB_tailscale_dir = INSTALLER_NAZZHUB_PERSISTENT_DIR + "/tailscale";
        if (type(entries) != "array" || !run_args([ "mkdir", "-p", NAZZHUB_tailscale_dir ])) {
        ...
        for (let entry in entries) {
            ...
            let target = NAZZHUB_tailscale_dir + "/" + entry;
            if (path_exists(target))
                continue;

            let temporary = NAZZHUB_tailscale_dir + "/." + entry + ".NAZZHUB-migrate";
...
function installer_post_install() {
    remove_globs(env("NAZZHUB_INSTALLER_LUCI_CACHE_GLOBS", "/var/luci-indexcache* /tmp/luci-indexcache*"));
    for (let path in [
        env("NAZZHUB_INSTALLER_LATEST_VERSION_CACHE", "/tmp/nazzhub.latest-version.cache"),
        env("NAZZHUB_INSTALLER_SYSTEM_INFO_CACHE", "/var/run/nazzhub/system-info.json"),
        env("NAZZHUB_INSTALLER_SERVER_COUNTRY_CACHE", "/var/run/nazzhub/server-country-cache.json"),
        env("NAZZHUB_INSTALLER_SING_BOX_VERSION_CACHE", "/var/run/nazzhub/ui-state/sing-box-version"),
        env("NAZZHUB_INSTALLER_TMP_SYSTEM_INFO_CACHE", "/tmp/nazzhub/system-info.json")
    ])
        remove_path(path);
    ...
    if (env("NAZZHUB_WAS_ENABLED", "0") == "1" && path_executable(INSTALLER_NAZZHUB_INIT))
        run_args([ INSTALLER_NAZZHUB_INIT, "enable" ]);

    if (env("NAZZHUB_WAS_RUNNING", "0") == "1" && path_executable(INSTALLER_NAZZHUB_INIT)) {
        if (!run_args([ INSTALLER_NAZZHUB_INIT, "start" ]) &&
            !run_args([ INSTALLER_NAZZHUB_INIT, "restart" ]))
            warn("Failed to start NAZZHUB after upgrade.\n");
    }
...
function asset_matches(name, kind, ext, version) {
    ...
    if (kind == "backend")
        return (name == "nazzhub_" + version + "." + ext || name == "NAZZHUB_" + version + "." + ext);
    if (kind == "app")
        return (name == "luci-app-nazzhub_" + version + "." + ext || name == "luci-app-NAZZHUB_" + version + "." + ext);
    if (kind == "i18n")
        return (name == "luci-i18n-nazzhub-ru_" + version + "." + ext || name == "luci-i18n-NAZZHUB-ru_" + version + "." + ext);
    return false;
}
...
install_json_ucode() {
    NAZZHUB_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
    NAZZHUB_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND_PACKAGE" \
    NAZZHUB_INSTALLER_LEGACY_CONFIG_ALT="$LEGACY_CONFIG_PACKAGE_ALT" \
    NAZZHUB_INSTALLER_DEADLINE_HELPER="$(install_deadline_helper_path)" \
    NAZZHUB_INSTALLER_COMMAND_RESULT="$TMP_DIR/installer-command" \
...
check_system() {
    ...
    if [ -n "$major" ] && [ "$major" -lt 24 ]; then
        fail "NAZZHUB requires OpenWrt 24.10 or newer"
    fi
...
detect_installer_language() {
    ...
    INSTALLER_LANG="en"
    if pkg_is_installed "luci-i18n-nazzhub-ru"; then
        INSTALLER_LANG="ru"
        return 0
    fi
...
resolve_NAZZHUB_release() {
    asset_ext="ipk"

    [ "$PKG_IS_APK" -eq 1 ] && asset_ext="apk"

    if [ "${INSTALL_MODE_TEST:-0}" -eq 1 ]; then
        warn "Запущен тестовый режим установки напрямую из ветки main..."
        if [ "$FETCHER" = "curl" ]; then
            exec sh -c "$(curl -fsSL "https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/main/install-test.sh")" -- "$@"
        else
            exec sh -c "$(wget -q -O - "https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/main/install-test.sh")" -- "$@"
        fi
        exit 0
    fi

    NAZZHUB_RELEASE_JSON="$(fetch_github_latest_release_json "$REPO_OWNER" "$REPO_NAME" 2>/dev/null || true)"
    NAZZHUB_RELEASE_TAG="$(printf '%s' "$NAZZHUB_RELEASE_JSON" | install_json_ucode release-tag 2>/dev/null || true)"
    if [ -z "$NAZZHUB_RELEASE_TAG" ]; then
        warn "Релиз ${REPO_OWNER}/${REPO_NAME} пока не найден на GitHub."
        warn "Автоматически переключаемся на тестовую установку напрямую из репозитория..."
        if [ "$FETCHER" = "curl" ]; then
            exec sh -c "$(curl -fsSL "https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/main/install-test.sh")" -- "$@"
        else
            exec sh -c "$(wget -q -O - "https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/main/install-test.sh")" -- "$@"
    ...
    NAZZHUB_BACKEND_URL="$(printf '%s' "$NAZZHUB_RELEASE_JSON" | install_json_ucode release-asset-url backend "$asset_ext" 2>/dev/null)"
    [ -n "$NAZZHUB_BACKEND_URL" ] || fail "The NAZZHUB release does not contain a NAZZHUB .$asset_ext package"

    NAZZHUB_APP_URL="$(printf '%s' "$NAZZHUB_RELEASE_JSON" | install_json_ucode release-asset-url app "$asset_ext" 2>/dev/null)"
    [ -n "$NAZZHUB_APP_URL" ] || fail "The NAZZHUB release does not contain a luci-app-nazzhub .$asset_ext package"

    NAZZHUB_BACKEND_NAME="$(basename "$NAZZHUB_BACKEND_URL")"
    NAZZHUB_APP_NAME="$(basename "$NAZZHUB_APP_URL")"
    NAZZHUB_PACKAGE_VERSION="$(printf '%s\n' "$NAZZHUB_BACKEND_NAME" | sed 's/^[nN][aA][zZ][zZ][hH][uU][bB]_//;s/\.ipk$//;s/\.apk$//')"

    NAZZHUB_I18N_URL=""
    NAZZHUB_I18N_NAME=""

    if [ "$NAZZHUB_I18N_REQUESTED" -eq 1 ]; then
        NAZZHUB_I18N_URL="$(printf '%s' "$NAZZHUB_RELEASE_JSON" | install_json_ucode release-asset-url i18n "$asset_ext" 2>/dev/null)"
        [ -n "$NAZZHUB_I18N_URL" ] || fail "The NAZZHUB release does not contain a luci-i18n-nazzhub-ru .$asset_ext package"
        NAZZHUB_I18N_NAME="$(basename "$NAZZHUB_I18N_URL")"
    fi
}
...
select_sing_box_installation() {
    ...
    if [ "$NAZZHUB_LEGACY_DETECTED" -eq 1 ] &&
        ...
        msg "The legacy binary-managed sing-box variant will be reinstalled for NAZZHUB"
        return 0
    fi
...
install_selected_sing_box() {
    ...
    [ -x /usr/bin/nazzhub ] || fail "NAZZHUB backend must be installed before sing-box component action"
    msg "Installing selected sing-box variant through NAZZHUB ucode backend"
    if ! /usr/bin/nazzhub component_action sing_box "$action" >"$output_file" 2>&1; then
...
cleanup_legacy_installation() {
    ...
    install_json_ucode installer-cleanup-legacy >"$state_file" ||
        fail "Failed to prepare the system before NAZZHUB package installation"
...
download_NAZZHUB_packages() {
    NAZZHUB_BACKEND_FILE="$TMP_DIR/$NAZZHUB_BACKEND_NAME"
    NAZZHUB_APP_FILE="$TMP_DIR/$NAZZHUB_APP_NAME"
    NAZZHUB_I18N_FILE=""

    download_with_retry "$NAZZHUB_BACKEND_URL" "$NAZZHUB_BACKEND_FILE" "$NAZZHUB_BACKEND_NAME" || fail "Failed to download $NAZZHUB_BACKEND_NAME"
    download_with_retry "$NAZZHUB_APP_URL" "$NAZZHUB_APP_FILE" "$NAZZHUB_APP_NAME" || fail "Failed to download $NAZZHUB_APP_NAME"

    if [ -n "$NAZZHUB_I18N_URL" ]; then
        NAZZHUB_I18N_FILE="$TMP_DIR/$NAZZHUB_I18N_NAME"
        download_with_retry "$NAZZHUB_I18N_URL" "$NAZZHUB_I18N_FILE" "$NAZZHUB_I18N_NAME" || fail "Failed to download $NAZZHUB_I18N_NAME"
    fi
}

install_backend_package() {
    pkg_install_files "$NAZZHUB_BACKEND_FILE" || fail "NAZZHUB installation failed"
}

migrate_legacy_configuration() {
    [ "$NAZZHUB_LEGACY_DETECTED" -eq 1 ] || return 0

    if [ -n "$LEGACY_CONFIG_BACKUP" ]; then
        cp "$LEGACY_CONFIG_BACKUP" /etc/config/nazzhub ||
            fail "Failed to restore the legacy configuration for migration"
        chmod 0644 /etc/config/nazzhub ||
            fail "Failed to set permissions on the NAZZHUB configuration"

        msg "Migrating the legacy configuration to NAZZHUB"
        if ! NAZZHUB_CONFIG_NAME="nazzhub" \
            NAZZHUB_LIB="/usr/lib/nazzhub" \
            ucode -L /usr/lib/nazzhub /usr/lib/nazzhub/config/migration.uc migrate-podkop; then
            cp "$LEGACY_CONFIG_BACKUP" /etc/config/nazzhub 2>/dev/null || true
            fail "Legacy configuration migration failed; the original configuration was restored"
        fi
    else
        warn "The legacy package had no readable configuration; NAZZHUB defaults will be used"
    fi
...
install_ui_packages() {
    pkg_install_files "$NAZZHUB_APP_FILE" || fail "luci-app-nazzhub installation failed"
...
post_install() {
    NAZZHUB_WAS_ENABLED="$NAZZHUB_WAS_ENABLED" NAZZHUB_WAS_RUNNING="$NAZZHUB_WAS_RUNNING" \
        ...
        fail "Failed to complete NAZZHUB post-install actions"
}
...
main() {
    ...
    msg "Source release: ${REPO_OWNER}/${REPO_NAME}@${NAZZHUB_RELEASE_TAG}"
    warn "Open LuCI and review your rules before enabling NAZZHUB"
}
