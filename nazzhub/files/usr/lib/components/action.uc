...
const BIN_PATH = getenv("FORKOP_BIN") || constants.FORKOP_BIN || "/usr/bin/nazzhub";
const SERVICE_INIT = getenv("FORKOP_SERVICE_INIT") || constants.FORKOP_SERVICE_INIT || "/etc/init.d/nazzhub";
...
function init_tmp_dir() {
    ...
    tmp_dir = trim(command_output_from_args([ "mktemp", "-d", "/tmp/nazzhub-updates.XXXXXX" ]));
    if (tmp_dir == "") {
        tmp_dir = "/tmp/nazzhub-updates." + owner_pid();
...
function make_tmp_file(prefix) {
    init_tmp_dir();
    let base = tmp_dir != "" ? tmp_dir + "/" + as_string(prefix) + ".XXXXXX" : "/tmp/nazzhub-updates-" + as_string(prefix) + ".XXXXXX";
...
function run_logged(description, command) {
    ...
    if (output_file == "")
        output_file = "/tmp/nazzhub-updates-command." + owner_pid();
...
function write_forkop_latest_version_cache(value, timestamp) {
    if (as_string(value) == "")
        return;
    write_file("/tmp/nazzhub.latest-version.cache", as_string(value) + "\n" + as_string(timestamp) + "\n");
}
...
function clear_version_caches() {
    remove_file("/tmp/nazzhub.latest-version.cache");
    remove_file(SYSTEM_INFO_CACHE_FILE);
    remove_file("/tmp/nazzhub/system-info.json");
}
...
function resolve_forkop_release(latest_version) {
    ...
    if (release_json == "")
        return null;
    let asset_ext = is_apk() ? "apk" : "ipk";
    let i18n_required = pkg_is_installed("luci-i18n-nazzhub-ru") ? "1" : "0";
