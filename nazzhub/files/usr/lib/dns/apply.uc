...
function dnsmasq_legacy_instance_exists() {
    return uci_exists("dhcp.NAZZHUB");
}
...
function dnsmasq_default_has_NAZZHUB_dns() {
    return list_has(dnsmasq_default_servers(), SB_DNS_INBOUND_ADDRESS);
}

function dnsmasq_has_NAZZHUB_dns() {
    return dnsmasq_default_has_NAZZHUB_dns() || dnsmasq_legacy_instance_exists();
}

function dnsmasq_has_NAZZHUB_managed_state() {
    return uci_get("dhcp.@dnsmasq[0].NAZZHUB_server") != "" ||
        uci_get("dhcp.@dnsmasq[0].NAZZHUB_noresolv") != "" ||
        uci_get("dhcp.@dnsmasq[0].NAZZHUB_cachesize") != "" ||
        uci_get("dhcp.@dnsmasq[0].NAZZHUB_notinterface") != "" ||
...
function dnsmasq_default_config_is_complete() {
    return dnsmasq_default_has_NAZZHUB_dns() &&
...
function dnsmasq_legacy_interfaces() {
    let legacy_dnsmasq_section = "NAZZHUB";
...
function backup_dnsmasq_server_list() {
    if (uci_get("dhcp.@dnsmasq[0].NAZZHUB_server") != "")
        return;

    for (let server in words(dnsmasq_default_servers())) {
        if (server != SB_DNS_INBOUND_ADDRESS)
            uci_add_list("dhcp.@dnsmasq[0].NAZZHUB_server", server);
    }
}
...
function dnsmasq_cleanup_legacy_instance() {
    ...
    uci_delete("dhcp.NAZZHUB");

    let backup_notinterfaces = uci_get("dhcp.@dnsmasq[0].NAZZHUB_notinterface");
    if (backup_notinterfaces != "") {
        ...
        uci_delete("dhcp.@dnsmasq[0].NAZZHUB_notinterface");
        return;
    }
...
function dnsmasq_configure_default_instance() {
    let default_has_NAZZHUB_dns = dnsmasq_default_has_NAZZHUB_dns();
...
function dnsmasq_restore_default_instance() {
    ...
    let backup_servers = uci_get("dhcp.@dnsmasq[0].NAZZHUB_server");
    ...
    if (backup_servers != "") {
        ...
        uci_delete("dhcp.@dnsmasq[0].NAZZHUB_server");
    }
    else {
    ...
    let noresolv = uci_get("dhcp.@dnsmasq[0].NAZZHUB_noresolv");
    if (noresolv != "")
        restore_dnsmasq_config_option("noresolv", "NAZZHUB_noresolv", "");
    else if (managed_global_dns)
        uci_set("dhcp.@dnsmasq[0].noresolv", "0");

    let cachesize = uci_get("dhcp.@dnsmasq[0].NAZZHUB_cachesize");
    if (cachesize != "")
...
function dnsmasq_configure(force) {
    ...
    if (as_string(force) != "force" && uci_get(CONFIG_NAME + ".settings.shutdown_correctly") == "0") {
        if (dnsmasq_default_config_is_complete()) {
            log("Previous NAZZHUB shutdown was unclean; dnsmasq already points to sing-box", "info");
            return true;
        }
        log("Previous NAZZHUB shutdown was unclean and dnsmasq is not ready; applying NAZZHUB DNS settings", "info");
    }
...
function dnsmasq_restore(force, quiet) {
    ...
    if (as_string(force) != "force" && uci_get(CONFIG_NAME + ".settings.shutdown_correctly") == "1") {
        if (!dnsmasq_has_NAZZHUB_dns()) {
            log("dnsmasq already uses non-NAZZHUB DNS settings; restore is not required", "info");
            return true;
        }
        log("NAZZHUB DNS settings are still present after a clean shutdown; restoring DNS settings in dnsmasq", "info");
    }
...
function failsafe_restore() {
    ...
    if (dnsmasq_management_disabled()) {
        if (!dnsmasq_has_NAZZHUB_managed_state()) {
            log("DNS rollback skipped: dont_touch_dhcp is enabled and no NAZZHUB dnsmasq changes were found", "info");
            return true;
        }

        log("Rolling back previous NAZZHUB dnsmasq changes because dont_touch_dhcp is enabled", "warn");
    }
    else {
        log("Rolling back NAZZHUB DNS changes in dnsmasq", "warn");
    }
