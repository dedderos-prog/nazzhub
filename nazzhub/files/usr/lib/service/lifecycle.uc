...
const BIN_PATH = getenv("FORKOP_BIN") || constant_value("FORKOP_BIN", "/usr/bin/nazzhub");
const SERVICE_INIT = getenv("FORKOP_SERVICE_INIT") || constant_value("FORKOP_SERVICE_INIT", "/etc/init.d/nazzhub");
...
const PERSISTENT_SUBSCRIPTION_CACHE_DIR = getenv("FORKOP_PERSISTENT_SUBSCRIPTION_CACHE_DIR") || "/etc/nazzhub/subscription-cache";
...
const RELOAD_LOCK_DIR = getenv("FORKOP_RELOAD_LOCK_DIR") || "/var/run/nazzhub.reload.lock";
const INTERNAL_CONFIG_TRIGGER_GUARD = getenv("FORKOP_INTERNAL_CONFIG_TRIGGER_GUARD") || "/var/run/nazzhub.internal-config-change";
...
const SB_VARIANT_STATE_FILE = constant_value("SB_VARIANT_STATE_FILE", "/etc/nazzhub/sing-box-variant");
const SB_VERSION_STATE_FILE = constant_value("SB_VERSION_STATE_FILE", "/etc/nazzhub/sing-box-version");
...
function uninstall() {
    ...
    if (fs.stat("/etc/init.d/nazzhub") != null) {
        command_success_from_args([ "/etc/init.d/nazzhub", "stop" ]);
        command_success_from_args([ "/etc/init.d/nazzhub", "disable" ]);
    }

    package_manager_remove_if_installed("luci-i18n-nazzhub-ru");
    package_manager_remove_if_installed("luci-app-nazzhub");
    ...
    command_success_from_args([ "rm", "-rf", "/usr/lib/nazzhub" ]);
    command_success_from_args([ "rm", "-rf", LUCI_VIEW_DIR ]);
    remove_file(SERVICE_INIT);
    remove_file(BIN_PATH);
    remove_file("/usr/share/luci/menu.d/luci-app-nazzhub.json");
    remove_file("/usr/share/rpcd/acl.d/luci-app-nazzhub.json");
    remove_file("/etc/uci-defaults/50_luci-nazzhub");
